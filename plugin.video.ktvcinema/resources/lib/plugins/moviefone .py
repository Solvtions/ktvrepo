...
def search_trailers(url):
    ...
    try:
        ...
        html = requests.get(url).content
        block = re.compile('search-head-one(.+?)text/javascript',re.DOTALL).findall(html)
        try:
            match = re.compile('"search-title">(.+?)</a>.+?"search-more-links"><a href="(.+?)">(.+?)</a>',re.DOTALL).findall(str(block))
            for name2,link2,key2 in match:
                if key2 == "Trailers":
                    url2 = link2
                    html2 = requests.get(url2).content
                    match2 = re.compile('"name": "(.+?)".+?"description": "(.+?)".+?thumbnailUrl": "(.+?)".+?"contentUrl": "(.+?)"',re.DOTALL).findall(html2)
                    for name,summary,thumbnail,link in match2:
                        name = name.replace("&#039;","")
                        name = remove_non_ascii(name)
                        summary = clean_search(summary)
                        xml += "<item>"\
